...
> 版本说明（截至 2026 年 2 月）：本章以 Android 16（API 36）为基线，兼顾 Android 14/15 的常见行为约束；涉及上架 targetSdk 要求请以 Google Play 官方政策为准。

安卓是一种以 Linux 为基础的开放源代码操作系统，主要用于移动和便携设备。当前稳定主版本为安卓 16（API 36），由谷歌领导的开放手机联盟（Open
Handset Alliance，OHA）持续演进。安卓 16 延续了 Material You / Material 3 设计体系，并继续强化大屏与折叠屏体验、后台任务与电量管理、隐私安全保护，以及系统组件模块化更新能力。
...
Machine），由C++开发；最上层是各种应用软件组成，包括通话程序、短信程序和游戏等。安卓系统作为一个移动开放平台，提供了与苹果移动系统不同的生态环境。任何手机厂商都可以免费使用安卓系统来定制自己的产品，而且安卓系统提供了标准的SDK，应用软件可以由第三方开发者独立完成，目前应用的开发语言主要使用 Kotlin（也可使用 Java）。
...
<tr class="even">
<td><a href="https://en.wikipedia.org/wiki/Android_16">安卓 16</a></td>
<td>巴克拉瓦</td>
<td>16</td>
...
注：上表“支持（安全修复）”仅用于粗略反映平台版本的新旧程度；实际安全更新周期以设备厂商/渠道与官方公告为准。
