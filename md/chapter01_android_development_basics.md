...
安卓提供开放的应用框架，应用开发者可以借助它访问系统能力、组织组件协作并构建界面交互。例如，定位、通知、后台任务、资源访问、窗口管理等能力，基本都通过这一层提供给应用。应用框架的价值在于把复杂的系统能力封装为稳定的 API，使开发者不必直接面对底层实现细节。应用框架主要由下面这些能力组成：
  ...
  - 内容提供器（ContentProvider）使应用程序能访问其他应用程序（如通讯录）的数据，或共享自己的数据。
...
Android Runtime包括核心库、ART和Dalvik虚拟机。从安卓 4.4开始，安卓就推出了新的Android
...
图 1‑16 Create New Project 向导入口
...
图 1‑21 布局编辑器中 activity_main.xml 的初始界面
...
图 1‑24 为 ImageView 选择 drawable 资源
...
图 1‑25 在 Attributes 面板中修改 ImageView 的 id
...
Plugin for Gradle”引入了需要的大多数 DSL 元素。开始新项目时，Android Studio会自动创建其中的部分文件，如图 1‑45 所示，并为其填充合理的默认值。
