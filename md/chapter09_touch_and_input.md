...
本章聚焦移动设备最具代表性的交互方式: 触摸输入。与桌面系统依赖鼠标和键盘不同，安卓应用的大量操作都直接发生在屏幕上，因此开发者既要理解单点、多点、滑动、缩放等手势本身，也要理解事件在 View 体系中的传递和消费过程。

> 版本说明（截至 2026 年）：本章示例按当前主流 Android 版本更新，重点兼容 Android 7.0+ 拖放 API 与 AndroidX 实践；文中旧接口会给出替代建议。

## 9.1 输入事件

在安卓中，截获用户输入并不是只有一种方法。对于标准控件，最常见的方式是给特定 `View` 注册事件监听器；对于自定义控件或更底层的交互，则常常需要重写 `onTouchEvent()`、`dispatchTouchEvent()` 等回调。也就是说，监听器更适合“在已有控件上补充行为”，而回调方法更适合“定义控件本身如何处理事件”。

理解这一点很重要，因为它直接决定后面该用哪种手段解决问题：如果只是想响应一个按钮点击，监听器通常就足够；如果要自己控制滑动、拖动、拦截和消费过程，就必须进一步理解 View 体系中的事件分发与处理链路。本节先从输入事件的基本入口讲起，再过渡到触摸事件的传递和消费机制。
...
码 9‑2 **在 Activity 中实现 OnClickListener 接口**
...
码 9‑3 **通过 nextFocusUp 与 nextFocusDown 定义焦点循环**
...
android:focusableInTouchMode，声明 View 对象在触摸模式下可聚焦。如需让特定 View 对象获得焦点，调用
...
当触摸事件发生时，`MotionEvent` 会作为参数被传递给应用中的相关方法。在安卓系统中，`ViewGroup`、`View`、`Activity` 及其子类都可能参与处理这一事件。与按钮点击这类单点交互不同，触摸事件往往涉及层级关系、事件序列和消费权判断，因此不能简单理解为“谁注册了监听器就一定先处理”。

真正需要弄清楚的是三个问题：事件先到谁手里、谁有权拦截、谁最终把它消费掉。只有把这条链路想清楚，才能解释诸如“父容器为什么抢走了子控件的滑动”“为什么 `onClick()` 没有触发”“为什么返回 `true` 后后续监听器收不到事件”这类典型现象。在复杂布局里，很多交互 Bug 本质上都不是控件错了，而是事件分发和消费关系没有设计清楚。
...
码 9‑4 **dispatchTouchEvent(MotionEvent ev) 方法签名**
...
码 9‑5 **onInterceptTouchEvent(MotionEvent ev) 方法签名**
...
码 9‑6 **onTouchEvent(MotionEvent ev) 方法签名**

这个方法可以用来处理触摸事件。该方法在View、ViewGroup以及Activity类中都有定义，并且所有的View子类全部重写了该方法，包括Layouts、Buttons、Lists、Surfaces、Clocks等等，这说明所有这些组件都可以使用触摸事件进行交互，应用程序可以通过该方法处理手机屏幕的触摸事件。下面这个示例用来观察触摸事件在界面层级中的传递路径：在一个自定义布局中包含自定义的TextView控件，并分别覆盖活动、自定义布局和TextView中的相关方法，通过日志输出观察事件流向。首先创建一个 `LinearLayout` 子类，覆盖前面提到的 `dispatchTouchEvent()`、`onInterceptTouchEvent()` 和 `onTouchEvent()`，处理在这个布局上发生的触摸事件（见码 9‑7）。
...
然后，定义用于测试的活动布局文件，在界面中明确给出触摸事件传递的层次关系（见码 9‑9）。
...
码 9‑9 **触摸事件传递示例的 XML 布局**

定义好布局文件后，创建一个活动，导入所定义的XML布局文件，运行应用程序，可以显示出图 9‑2 所示的界面。
...
在上面示例中，活动和自定义布局中的三种相关方法返回值都为false，则触摸事件会传递到最内层的自定义TextView。由于自定义TextView的onTouchEvent()返回值为true，所以触摸事件被消费。图 9‑3 示意了从外向内传递触摸事件的路径。
...
码 9‑10 **触摸事件从外向内传递时的日志输出**

如果上面例子中自定义TextView的onTouchEvent()方法返回了false，则接下来的触摸事件，即ACTION\_DOWN这一事件序列的后续触摸事件，就不会再传递到这个View或者ViewGroup元素，而会在其父元素终止，并调用其父元素的onTouchEvent()。这就是说，如果最内层界面元素的onTouchEvent()方法返回了false，则事件会自内向外再次传递，直到某个界面元素的onTouchEvent()方法返回true。这时，此事件序列的后续触摸事件也会直接由这个界面元素的onTouchEvent()方法处理。如果自内向外的所有View或者ViewGroup界面元素的onTouchEvent()方法都返回false，那么ACTION\_DOWN这一事件序列的后续触摸事件最后会由活动处理，即调用活动的onTouchEvent()方法，并最终结束触摸事件的传递，这就是从内向外的触摸事件传递过程。还是使用上面的例子，只是在码 9‑5 中，将自定义TextView的onTouchEvent()方法返回值改为false，触摸事件就会从内向外传递（见图 9‑4 虚线所示）。
...
码 9‑12 **OnTouchListener.onTouch() 方法签名**
...
event)。下面这个例子只聚焦一个目标：把触摸点移动速度写入日志，以便读者观察 VelocityTracker 的计算结果。具体实现见码 9‑10，其中touch\_velocity\_tracker.xml布局文件中可以只简单设置一个TextView。
...
码 9‑13 **使用 VelocityTracker 进行速率跟踪**
...
如果希望使用多点触控，首先要知道使用getPointerCount()方法获取当前屏幕上的触点数量。只要获得的触点数量大于1的话，就需要处理触点索引和触点ID。MotionEvent对象中包含了当前从索引为0开始的触点信息，一直到getPointerCount()方法返回的最大索引值。触点索引始终从0开始，如果有三个触点，则它们的索引分别为0、1和2。调用类似getX()的方法必须使用触点索引作为参数，才可以获得指定的触点信息。触点ID也是一个整数，可以表示哪个触点被跟踪。当第一个手指触摸屏幕时，触点ID也是从0开始的，但是随着手指从屏幕上的移走或放下，当前这个值就有可能不是从0开始的了。这是由于安卓系统使用触点ID来跟踪屏幕上不同手指的运动，触点ID可以固定地指定某个手指。为了说明这一点，假设由两个手指产生的一对触摸序列。序列产生的过程为：首先从手指1触摸开始，然后是手指2触摸；接着手指1移走，然后手指2移走。手指1触摸时，会得到触点ID为0，手指2触摸时，会得到触点ID为1，而它们的触点索引也相同。当手指1移走时，手指2的ID仍然为1，而这时手指2的触点索引就会变成0，这就是上面说的触点索引始终从0开始。这样就可以在应用程序里使用触点ID将触摸事件与特定的手指关联起来，以及涉及到的其他手指。在一个手势中，可以使用getPointerId()方法获得触点ID，用来在后续的触摸事件中跟踪手指。发生一系列动作后，可以使用findPointerIndex()方法找到触点ID当前对应的触点索引，再使用触点索引获取触摸事件的信息（见码 9‑14）。
...
码 9‑14 **使用 getPointerId() 跟踪活动触点**

另外，可以使用getActionMasked()方法获得触摸事件的动作。与getAction()方法不同，这个方法是为多点触控定义的。这个方法的返回结果经过掩码处理，去掉了触点索引的信息。可以使用getActionIndex()方法返回触摸事件的触点索引（见码 9‑15）。
...
码 9‑15 **使用 getActionMasked() 识别多点触控动作**
...
当初始化GestureDetector对象时，需要传入一个实现了OnGestureListener接口的参数，当一个特定的手势被识别时，就会执行OnGestureListener中的各种手势处理方法。为了使GestureDetector对象能够接收触摸事件，需要覆盖View或活动的onTouchEvent方法，并将所有触摸事件传递到GestureDetector对象中（见码 9‑16）。
...
码 9‑16 **使用 GestureDetectorCompat 识别常见手势**

在码 9‑16 中，如果OnGestureListener中方法执行的结果为true，表明已经处理触摸事件；如果为false，则触摸事件继续传递，直到成功处理为止。如果只想处理部分手势，可以继承SimpleOnGestureListener。SimpleOnGestureListener实现了OnGestureListener接口的所有方法，而且返回值都为false。因此只需要覆盖那些关心的方法就行了。不论是否使用OnGestureListener，最好的做法就是实现onDown()方法，并返回true。这是因为所有的手势都需要判断onDown()方法的返回值，如果在onDown()中返回false（这是SimpleOnGestureListener中的默认返回结果），系统认为忽略剩余的手势动作，SimpleOnGestureListener中的其他方法不会被调用。

### 9.6.2 缩放手势

从安卓 2.2 开始引入了ScaleGestureDetector类，这个可以用来识别缩放手势。缩放手势有两种操作，一种是两个手指同时触摸屏幕，向相互远离的方向移动，然后同时离开屏幕，这是放大的操作；另一种是两个手指同时触摸屏幕，向相互靠近的方向移动，然后同时离开屏幕，这是缩小操作。下面这个例子聚焦“如何把缩放手势映射到图标缩放”。第一步先定义 XML 布局文件（见码 9‑17）。
...
码 9‑17 **scale\_detector\_layout.xml：缩放手势示例布局**

码 9‑17 布局文件中使用ImageView控件定义一个图片源，而且定义了一种矩阵缩放的方式。这个图片的大小是填充布局，这样图片不会被ImageView的边界剪切。码 9‑18 是具体实现缩放手势的活动代码。
...
码 9‑18 **ScaleDetectorActivity：处理双指缩放手势**

码 9‑18 在onCreate()中，获取了ImageView和ScaleGestureDetector对象。覆盖活动的onTouchEvent()方法。在这个方法中，将所有触摸事件传递给ScaleGestureDetector的onTouchEvent()方法，并且结果返回true。这样活动的onTouchEvent()方法可以不断地获得新的事件，并把事件传递给ScaleGestureDetector，通过所获得所有的触摸事件，识别出缩放手势。例子中，在自定义ScaleListener的onScale()方法中实现图片的缩放。实际上，OnScaleGestureListener监听器中有onScaleBegin()、onScale()和onScaleEnd()三个回调方法，分别表示手势开始、进行和结束三个过程。

在onScale()方法中传入ScaleGestureDetector对象，可以得到很多关于缩放操作的信息。mScaleFactor是缩放因子，在1的上下区间（最小值为0.1，最大值为5.0）浮动。当两个手指靠近时，此值会小于1；当两个手指分开时，此值会大于1。mScaleFactor的值从1开始，随着手指的靠近或分开，图片逐渐变小或变大。如果mScaleFactor等于1，图片是正常的大小。还为mScaleFactor设置了最小值和最大值，防止图片过小或过大。

## 9.7 拖放处理

拖放是触摸交互中的一个综合场景，因为它同时涉及手势识别、事件传递、拖动阴影绘制和目标视图的接收逻辑。安卓为此提供了专门的拖放框架，可通过 `View.OnDragListener` 或 `onDragEvent()` 来接收拖动事件。对于新项目，通常应优先使用 `startDragAndDrop()`；只有在兼容旧版设备时才需要回退到 `startDrag()`。

从本质上看，拖放框架并不只是“把一个控件拖着走”，而是通过一次图形化手势，把某段数据或某个操作上下文从一个视图传递给另一个视图。因此，它既可以用于移动界面对象，也可以用于触发放置区响应、跨视图传递内容，或者完成更复杂的交互组合。
...
用户继续拖动。当拖动阴影和某个视图对象的边界框发生相交时，系统会向已注册的拖动监听器发送后续拖动事件。作为回应，目标视图可以根据事件类型改变自身外观，例如在阴影进入边界框时高亮显示可放置区域。这里常见的进入事件是 `ACTION_DRAG_ENTERED`，而不是拖放结束事件。
...
当拖动事件发生时，系统创建一个[DragEvent](http://developer.android.com/reference/android/view/DragEvent.html)对象，并传递给相应的回调方法或监听器。这个对象包括拖放事件中正在发生事件的类型以及其他依赖这个事件类型的数据。监听器调用[getAction()](http://developer.android.com/reference/android/view/DragEvent.html#getAction\(\))这个方法就可以获得这个事件类型（见表 9‑2）。
...
码 9‑19 **创建 ClipData 并启动拖动**

  - 创建拖动阴影

创建一个View.DragShadowBuilder的子类MyDragShadowBuilder，覆盖onDrawShadow()方法和onProvideShadowMetrics()方法，为拖动一个TextView创建一个小的灰色矩形拖动阴影（见码 9‑20）。
...
上一节介绍了如何在拖放操作的各个阶段对事件进行处理，下面这个例子把完整拖放流程串起来，帮助读者看到“源视图、拖放阴影、目标容器、事件回调”是如何协同工作的。实现步骤共有六步，实际项目中可以按需求裁剪。

1.  定义XML绘制图片

在res的drawable目录下，创建 shape.xml 文件，作为正常状态下的背景资源（见码 9‑21）。
...
码 9‑21 **shape.xml：定义默认拖放背景**

在res的drawable目录下，创建 shape\_droptarget.xml 文件，作为被拖动对象进入目标区域后的高亮背景（见码 9‑22）。
...
码 9‑22 **shape\_droptarget.xml：定义拖放目标高亮背景**

2.  定义布局等资源文件

定义活动显示的用户界面 main.xml 布局文件。界面使用网格布局，每个网格都以前面定义的 shape 资源作为背景，并放置一个图片（见码 9‑23）。
...
创建 DragActivity 作为主界面，并在其中加载 main.xml 布局文件（见码 9‑24）。
...
码 9‑24 **DragActivity 的初始化代码**

（4）定义或实现TouchListener

在 DragActivity 中自定义一个 OnTouchListener 监听器 MyTouchListener。当接收到 ACTION\_DOWN 事件时，它会创建一个 ClipData 对象，并使用 View.DragShadowBuilder 为当前触摸的对象创建拖动阴影，从而启动拖动操作（见码 9‑25）。
...
码 9‑25 **启动拖动操作的 OnTouchListener**

在这里也可以使用自定义OnLongClickListener来设置开始拖动操作，在其onLongClick()方法中创建ClipData对象、创建拖动阴影，以及进行其他的相关初始设置。

（5）定义或实现DragListener

在DragActivity中自定义一个OnDragListener的监听器MyDragListener，定义在拖动操作时，各事件发生时视图对象的工作（见码 9‑26）。当ACTION\_DRAG\_STARTED发生时，因为前面在onTouch()代码中已经做了拖动开始操作时的一些处理，这里就不做任何处理了；当被拖动的对象进入本视图对象的ACTION\_DRAG\_ENTERED事件发生时，将背景改变为shape\_droptarget.xml所定义的图片；当拖动释放的ACTION\_DROP事件发生时，将原来的拖动对象删除，加入到所拖动到的位置；当拖动操作结束的ACTION\_DRAG\_ENDED和ACTION\_DRAG\_EXITED事件发生时，将背景改回shape.xml定义的正常背景。
...
本章围绕触摸屏交互展开，介绍了输入事件、触摸事件序列、事件传递机制、速率跟踪、多点触控、手势识别以及拖放处理等内容。核心知识点不只是各种 API 的名称，而是如何把用户的连续动作理解为一组可被系统识别、分发和消费的事件。

在实际开发中，只有把 MotionEvent、事件分发链路和具体手势处理机制结合起来理解，才能正确判断事件应该由哪个 View 消费、何时需要拦截、何时需要继续向下传递。本章建立的是触摸交互的底层认识，它会直接影响自定义控件、复杂手势和拖放场景的实现质量。
