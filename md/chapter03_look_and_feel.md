...
本章从“界面如何响应用户操作、如何呈现视觉反馈”两个角度展开，重点介绍安卓事件处理机制与常用控件的使用方法。通过这些内容，读者可以把上一章中关于布局和界面骨架的知识，进一步落实为真正可交互、可反馈的界面。

> 版本说明（截至 2026 年）：本章示例已按 AndroidX 与现代 Android UI 实践补充更新，涉及进度反馈、选择器、可绘制对象与质感设计相关内容。

## 3.1 事件处理

无论应用使用 Java 还是 Kotlin 编写，只要用户点击按钮、输入文字、滑动屏幕或触发系统状态变化，界面层都需要把这些操作转换为程序可处理的事件。安卓所谓的事件处理，本质上就是“系统捕获用户动作或状态变化，然后把事件分发给合适的组件和代码去响应”的过程。

从学习路径上看，本章主要关注两类事件处理方式：一类是基于监听接口，把特定事件委托给注册好的监听器对象；另一类是基于回调机制，由组件或框架在特定时机反向调用预定义方法。二者并不是互相排斥的关系，而是构成了安卓界面交互的两种基础组织方式。
...
码 3‑3 **在 XML 布局中通过 android:onClick 绑定点击处理方法**

码 3‑3 在界面布局文件中为按钮绑定一个事件处理方法 clickHandler()，这就意味着开发者需要在对应活动中提供同名处理函数。下面的 Java 代码示例展示了这种“在 XML 中声明、在活动中实现”的绑定方式：
...
安卓提供了多种按钮类控件，包括基本的 `Button`、`ImageButton`、`ToggleButton`、`CheckBox` 和 `RadioButton` 等。虽然这些控件在外观和交互方式上各有差异，但它们的共同点是都承担“接收用户明确操作并触发界面行为”的职责，因此是事件处理最常见的入口之一。

按钮控件通常与单击事件紧密关联。对于基本按钮，既可以通过 `setOnClickListener()` 在代码中注册点击监听器，也可以在 XML 布局中使用 `android:onClick` 属性声明回调方法。实际工程中，显式注册监听器通常更清晰、更便于维护；而 XML 回调方式更适合用来演示最简单的交互流程。下面结合不同按钮类型，说明它们在属性设置和事件处理上的差异。
...
如果设置选项只有两种状态，可以使用开关按钮ToggleButton（见图 3‑6 左图）。安卓 4.0（API
级别14）提供了另外一种叫做Switch的开关按钮，这个按钮提供一个滑动控件，可以通过添加Switch对象来实现（见图 3‑6 右图）。
...
图 3‑6 ToggleButton 和 Switch

ToggleButton和Switch控件都是CompoundButton组合按钮的子类并且有着相同的功能，所以可以用同样的方法来实现它们功能。当用户选择ToggleButtons和Switch时，对象就会接收到相应的点击事件。要定义这个点击事件的响应操作，添加android:onClick属性到XML布局文件的开关按钮控件中去。例如，码 3‑17 定义了一个ToggleButton开关按钮，并设置了android:onClick点击响应属性。
...
与其他图形控件一样，除了在布局文件中定义之外，也可以通过代码的方式，为控件注册一个事件监听器。码 3‑19 说明了为ToggleButton注册监听器的具体实现代码：首先创建一个CompoundButton.OnCheckedChangeListener对象，覆盖OnCheckedChangeListener接口的抽象方法onCheckedChanged()，在其中实现点击ToggleButton对象后的事件处理，然后通过调用该ToggleButton对象的setOnCheckedChangeListener()方法，将监听器绑定到按钮上。
...
码 3‑21 的最后两行也可以压缩成链式写法，从而避免显式创建 Toast 对象。下面这行代码展示了这种更紧凑的写法：
...
安卓中用于文本显示和编辑的控件主要包括TextView和EditText两种。实际上，很多控件都继承自TextView类，包括Button、CheckTextView、EditText等，但在文本显示与输入场景中最常用的仍然是TextView和EditText，因此这里主要介绍这两个控件如何定义和使用。
...
码 3‑26 **TextViewModifyByCodeActivity.java：以代码方式修改 TextView**

通过上面的尝试，说明通过Java代码程序和XML布局文件都可以实现TextView属性的设置。不过在安卓应用系统开发过程中，还是推荐使用XML进行布局和界面外观的设计，使用Java程序代码实现程序逻辑。

### 3.4.2 EditText

EditText是安卓的文本编辑框，是用户和安卓应用进行数据交互的窗口，可以接受用户的文本数据输入，并将其传送到应用程序中。EditText是TextView的子类，所以EditText继承了TextView的所有方法和所有属性。EditText类似于Java图形界面的文本编辑框，但与后者相比，增加从TextView继承的属性之后，设置EditText的显示和输入时，就可以根据不同的需求设计出更加有个性和特点的交互界面。例如，可以通过EditText的属性设置文本编辑框的最大长度、空白提示文字等，或者限制输入的字符类型只能为电话号码。表 3‑1 中列出了EditText常用的一些属性和说明，这些属性也同样适用于TextView。
...
码 3‑27 **edittext\_layout.xml：EditText 属性示例布局**
...
码 3‑28 **EditTextSearchActivity.java：读取输入并执行搜索**

## 3.5 图像控件

ImageView是安卓中用于显示图片的控件，可以显示来自资源文件、Drawable对象、Bitmap对象或ContentProvider URI等不同来源的图片，并能够通过各种属性控制图片的显示效果，例如缩放和着色等。ImageView的属性既可以直接在XML布局文件中设置，也可以在Java应用程序中动态修改。作为View的子类，它在XML布局文件中的基本设置方式与TextView类似，见码 3‑29。
...
=
@drawable/my\_image"。另一个问题是如何通过ImageView的属性，来控制原始图片的尺寸、比例或者显示位置，以匹配ImageView本身设置的大小，运行时显示出设计预想的效果。解决方式是，使用ImageView最重要的一个属性android:scaleType，根据需要设置对应的值。表 3‑2 中列出了ScaleType的属性值和对应的含义。
...
图 3‑11 不同 ScaleType 属性的 ImageView 显示效果
...
码 3‑35 **ProgressDialogActivity.java：显示进度对话框**
...
public class SpinnerPlanetActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.spinner_planet);
...
码 3‑39 **SpinnerPlanetActivity：使用 Spinner 展示行星列表**

![](media/chapter03_look_and_feel/media/image14.png)

图 3‑13 SpinnerPlanet 示例运行界面
...
码 3‑40 **TimePickerFragment：选择时间**
...
码 3‑41 **DatePickerFragment：选择日期**
...
码 3‑42 **date\_time\_picker.xml：日期与时间选择示例布局**
...
前面几节主要介绍了标准控件和常规交互方式，本节进一步补充一些能够明显提升界面表现力的小技巧。它们未必都是大型项目的核心能力，但往往能帮助读者更好地理解文本渲染、Drawable 组织和视觉反馈这些看似细碎、实际很常见的界面问题。
...
<LinearLayout
    ...
    <TextView
        ...
        android:text="@string/android_develop "
...
使用活动导入码 3‑46 中的布局文件，察看显示内容可知，在字符串中定义的格式效果完全显示出来。

  - 动态方式设置文本样式

也就是通过应用程序代码编程的方式，来设置或改变TextView中的显示内容的格式。这种方式比静态方式要复杂，但是更具灵活性。下面的代码示例先把 TextView 的内容切换为 Spannable 缓冲类型，为后续动态附加样式做准备：
...
完成上述设置后，TextView 中的内容就以 Spannable 形式保存，此时可以直接通过 getText() 取回可编辑的样式对象。下面的代码示例演示这一读取动作：
...
如果不想直接操作 TextView 当前内容，也可以先独立创建一个 SpannableString，再整体交给 TextView。下面的代码示例展示了这种写法：
...
下面的代码示例把文本字符串的第 2、3 个字符设置为粗斜体，用来说明 span 的起止位置与样式对象是如何配合的：
...
下面的码 3‑51 示例了以动态方式设置字符串中不同样式的用法。
...
public class SpannableActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.stlye_spannable);
...
码 3‑52 **auto\_complete.xml：自动补全输入布局**
...
下面的活动代码示例把建议词数组交给 ArrayAdapter，并通过 setAdapter() 挂接到 AutoCompleteTextView，从而启用自动补全列表：
...
码 3‑53 **AutoCompleteTextViewActivity：自动补全示例**
...
表 3‑3 bitmap 可绘制对象的 gravity 常量值
...
表 3‑4 bitmap 可绘制对象的 tileMode 取值
...
表 3‑7 clip/inset 类可绘制对象的 gravity 常量值
...
质感设计（Material Design）是谷歌提出的一套跨平台设计语言，用于统一界面在视觉、动效和交互层面的表达方式。对现代 Android 项目而言，真正落地时通常依赖 Material Components 或 Material 3 设计体系，而不仅仅是换一个主题颜色。理解这部分内容的关键，在于认识到“外观与感觉”并不是单个控件的修饰，而是一整套关于层级、反馈、导航与一致性的设计规则。

如需在安卓应用中落实质感设计，需要同时关注设计准则和实现手段：前者回答“界面应该呈现怎样的视觉逻辑”，后者回答“该用哪些组件、主题和动画能力来实现”。安卓提供了以下能力来帮助构建符合质感设计思路的应用：
...
图 3‑21 视图 Z 高度对阴影与层级的影响

当布局包含卡片式内容时，通常会通过高度、圆角、阴影和内边距来建立信息层级。在传统视图体系中，可以使用 `CardView` 或 `MaterialCardView` 来实现这类界面；在更新的工程实践中，也可以通过 Material 3 组件或 Compose 对应组件实现相同思想。借助动画 API，还可以为轻触反馈、状态切换和页面过渡提供更自然的视觉反馈。这些能力包括：
...
本章围绕界面交互体验中的“输入响应”和“视觉呈现”展开。前半部分介绍了基于监听接口和基于回调机制的事件处理思路，帮助读者理解事件源、事件对象和监听器之间的关系；后半部分结合按钮、提示、文本、图像、进度条、微调框、选择器以及可绘制对象等内容，说明常用控件的属性配置与交互处理方式。

从学习路径上看，本章的重点不只是会用某个控件，更重要的是理解控件行为与事件分发之间的联系。只有同时掌握事件处理机制、控件属性和视觉资源组织方式，才能在实际项目中构建既可用又具有一致体验的安卓界面。
