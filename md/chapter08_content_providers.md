...
> 版本说明（截至 2026 年 2 月）：本章按当前主流实践更新，并以 Android 16（API 36）为基线，重点兼容 Android 12+ 组件导出规则与现代权限模型；文中历史接口会附迁移建议。
...
```text
内容提供器在用户字典中使用android.permission.WRITE_USER_DICTIONARY权限来控制对数据的插入，更新和删除。为获得访问内容提供器的权限，应用程序在Manifest文件中需要使用<uses-permission>标签；对于危险权限，还需要在运行时向用户申请授权。需要注意的是，用户词典相关权限在新版本系统中不再适合作为普通第三方应用的通用示例。
...
// 首先向RawContacts.CONTENT_URI执行一个插入，
