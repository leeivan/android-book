...
如需访问内容提供程序中的数据，客户端通常不会直接接触 `ContentProvider` 实例，而是通过 `Context` 提供的 `ContentResolver` 与其通信。可以把 `ContentResolver` 理解为“系统代办层”：客户端把 URI、查询条件和修改请求交给它，它再去找到正确的提供器并执行对应方法。对调用方来说，最常用的操作就是标准的 CRUD：查询、插入、更新和删除。
...
Manifest文件中的权限声明元素是\<uses-permission\>元素，权限的值从将要访问内容提供器所定义的权限中选择，根据需求指定准确的权限名称。例如，读取联系人数据通常需要声明 `android.permission.READ_CONTACTS`，代码如下：
