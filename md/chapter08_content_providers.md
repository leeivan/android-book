...
本章讨论应用数据如何在跨组件、跨进程甚至跨应用的场景下被安全地共享和访问。前一章主要解决“数据如何存起来”，而本章进一步回答“存好的数据如何通过统一接口对外提供”。这也是理解内容提供器价值的关键。

> 版本说明（截至 2026 年）：本章按当前主流实践更新，重点兼容 Android 12+ 组件导出规则与现代权限模型；文中历史接口会附迁移建议。

在安卓上，应用默认对自己的文件和数据库拥有私有访问权。如果希望把其中一部分数据安全地开放给其他组件或其他应用，就需要借助内容提供器定义统一的访问入口。内容提供器不仅提供标准化的查询和修改接口，还承担了 URI 标识、权限边界和进程间数据访问的组织职责。

## 8.1 基本概念

一般情况下，安卓应用的数据默认是私有的，其他应用不能直接访问。如果希望把其中一部分数据安全地开放出去，就需要定义一个统一、可控制、可授权的数据访问入口，这正是内容提供器存在的意义。它解决的不是“怎样把数据存到数据库里”，而是“怎样让外部以受控方式访问这些数据”。

从系统设计角度看，内容提供器更像是一份访问契约，而不是某种具体存储格式。它可以对外表现得像一张表，也可以封装文件、数据库、缓存甚至网络同步后的本地镜像数据。对调用方而言，最重要的是知道该使用什么 `content://` URI、能执行哪些查询或修改操作、需要什么权限；至于数据最终保存在 SQLite、文件系统还是其他结构中，则由提供器内部自行决定。下面用一张二维表的形式帮助理解这种“对外统一、对内可变”的接口思想。
...
表 8‑1 用户词典内容提供器的示例数据结构

在表 8‑1 中，每行代表了一个不能在标准字典中找到的词，每一列代表了这个词的一个属性。第一行的是存储在内容提供器中的列名称。在这个内容提供器中，\_ID列作为主键列，由内容提供器自动管理维护。主键对于一个内容提供器并不是必须具备的，即使有主键，内容提供器也不必一定要使用\_ID作为主键的列名。但是，如果要把内容提供器中的数据通过用户界面显示出来，常常需要把内容提供器绑定到一个叫作ListView的用户界面控件上，这就必须有一个列名叫作\_ID。在后面讨论显示查询结果的部分，会对此有详细的解释。

内容提供器有助于应用管理其自身和其他应用所存储数据的访问，并提供与其他应用共享数据的方法。它们会封装数据，并提供用于定义数据安全性的机制。内容提供器是一种标准接口，可将一个进程中的数据与另一个进程中运行的代码进行连接。实现内容提供器大有好处。最重要的是，通过配置内容提供器，可以使其他应用安全地访问和修改应用数据（如图 8‑1 所示）。

<table>
<tbody>
<tr class="odd">
<td><p><img src="media/chapter08_content_providers/media/image1.png" style="width:2.13194in;height:1.63555in" /></p>
<p>图 8‑1 内容提供器如何管理存储空间访问</p></td>
<td><p><img src="media/chapter08_content_providers/media/image2.png" style="width:3.00347in;height:1.65287in" /></p>
<p>图 8‑2迁移内容提供器存储空间</p></td>
</tr>
</tbody>
</table>

如果计划共享数据，则可使用内容提供器。如果不打算共享数据，也可使用内容提供器，因为它们可以提供很好的抽象，但无需如此。此抽象可让修改应用数据存储实现，同时不会影响依赖数据访问的其他现有应用。在此情况下，受影响的只有内容提供器，而非访问该内容提供器的应用。例如，可以将
SQLite 数据库换成其他存储空间（如图 8‑2 所示）。许多其他类依赖于 ContentProvider 类：
...
如果正在使用以上某个类，则还需在应用中实现内容提供器。注意，使用同步适配器框架时，还可使用另一种方案：创建存根内容提供器。如需了解有关此主题的更多信息，参阅创建存根内容提供器。此外，在下列情况下，需要自定义内容提供器：
...
安卓框架内的某些内容提供器可管理音频、视频、图像和个人联系信息等数据。android.provider
软件包参考文档中列出了其中的部分提供程序。虽然存在一些限制，但任何安卓应用均可访问这些提供程序。内容提供器可用于管理对各种数据存储源的访问，包括结构化数据（如
SQLite
关系型数据库）和非结构化数据（如图像文件）。内容提供器可精细控制数据访问权限。可以选择仅在应用内限制对内容提供器的访问，授予访问其他应用数据的权限，或配置读取和写入数据的不同权限。可以使用内容提供器将细节抽象化，以用于访问应用中的不同数据源。例如，应用可能会在
SQLite
数据库中存储结构化记录，以及视频和音频文件。如果在应用中实现此开发模式，则可使用内容提供器访问所有这类数据。另注意，CursorLoader
对象依赖于内容提供器来运行异步查询，进而将结果返回至应用的界面层。如需了解有关使用 CursorLoader
在后台加载数据的更多信息，参阅使用 CursorLoader 运行查询。

应用程序使用访问提供器客户端对象来访问内容提供器的数据，可以称其为内容访问器。内容访问器的方法提供了基本的“CRUD”（创建，检索，更新和删除）数据存储的功能。因此在使用其他应用程序定义的内容提供器数据时，对于应用程序来说内容访问器的对象只是一个访问器，用于与内容提供器连接以及传递操作和参数，并不需要知道数据的具体存储结构，也不需要编码实现功能，具体的访问操作由内容提供器子类的同名方法来完成，这样其他应用程序只要了解内容提供器的数据二维表，就可以完成数据交互了。当然如果要访问内容提供器，应用程序必须在Manifest文件中添加特定的权限。这些将在内容提供器
...
URI主要分三个部分：scheme、authority 和path。其中authority又分为host和port（见图 8‑3）。

![](media/chapter08_content_providers/media/image3.png)

图 8‑3 通用资源标识符 URI 结构
...
称为scheme，对应图 8‑3 中的 A 部分，表示资源的类型。如果是网站资源则为”HTTP://”，Content
URI则为”content://”。

  - 资源名称

称为authority，对应图 8‑3 中的 B 部分，表示资源的唯一名称。如果是网站资源则为域名，Content
URI中则是内容提供器的唯一标识名称，可以使用包和类名来定义内容提供器资源名称。

  - 路径

称为path，对应图 8‑3 中的 C 和 D 部分，表示资源中的数据。如果是网站资源则为服务器上网页的路径名，Content
URI中则是内容提供器中的表名或指定表中的记录。例如图 8‑3 中C和D都是用来指定路径，C一般用来指定数据库中表的名字，这里指出使用trains表中的数据，D可以用来指定数据库中的一条记录，这里指定了ID为122的记录，如果没有指定ID，就表示返回全部记录。
...
对调用方而言，内容提供器通常会以一个或多个“类似表”的集合对外暴露数据。这样做的好处是，外部应用可以用统一方式理解行、列、URI、筛选条件和返回游标，而不必了解提供器内部到底怎样存储数据。也正因为这种统一接口存在，内容提供器可以和许多框架组件协同工作（如图 8‑4 所示），其中包括：
...
<p>图 8‑4内容提供器与其他组件的关系</p></td>
<td><p><img src="media/chapter08_content_providers/media/image5.png" style="width:1.64583in;height:1.82155in" /></p>
<p>图 8‑5 内容提供器、其他类和存储空间之间的交互</p></td>
</tr>
</tbody>
</table>

如需访问内容提供器中的数据，客户端通常不会直接接触 `ContentProvider` 实例，而是通过 `Context` 提供的 `ContentResolver` 与其通信。可以把 `ContentResolver` 理解为“系统代办层”：客户端把 URI、查询条件和修改请求交给它，它再去找到正确的提供器并执行对应方法。对调用方来说，最常用的操作就是标准的 CRUD：查询、插入、更新和删除。

从界面层访问内容提供器时，历史上常见做法是配合 `CursorLoader` 做异步查询；在现代项目里，也可以结合协程、Repository 或其他数据层封装来组织调用。但无论用哪种外层模式，底层都仍然离不开 `ContentResolver + content:// URI` 这一基本访问路径。

应用程序通过内容访问器对象访问内容提供器时，所使用的方法会调用内容提供器具体子类中的同名方法。例如，下面这个查询通过 `ContentResolver.query()` 从用户字典读取单词及其语言环境列表（见码 8‑1）。这个 `query()` 调用最终会转发到用户字典内容提供器中实现的 `ContentProvider.query()` 方法。
...
表 8‑2 query(Uri, projection, selection, selectionArgs, sortOrder) 与 SQL SELECT 的对应关系
...
能够从内容提供器中获取数据的前提，首先是所访问的内容提供器允许其他应用程序的读访问。从内容提供器中获取数据，应用程序需要“读权限”。在现代 Android 中，权限通常分为 normal 和 dangerous：normal 权限在安装时自动授予；dangerous 权限除了在 Manifest 中声明，还必须在运行时向用户申请授权。
...
下面继续以用户词典提供器为例，展示如何使用 `ContentResolver.query()`。提供器客户端查询与 SQL 查询类似，包含需要返回的列、选择条件和排序顺序。对于指定待检索行的表达式，这里将其拆分为选择子句和选择参数：如果用户未输入字词，选择子句会设为 `null`，查询返回提供器中的所有字词；如果用户输入了字词，选择子句就会设为 `UserDictionary.Words.WORD + " = ?"`，并把用户输入写入 `mSelectionArgs`。定义好这些变量后，就可以继续编写获取数据的方法了。
...
码 8‑7 **与 ContentResolver.query() 对应的 SQL 查询示例**
...
ContentResolver.query()方法执行后，返回的是游标对象，这是一个查询结果集合。如果遍历这个游标对象，就可以读取结果集中的所有数据，输出查询结果。码 8‑10 中使用Cursor的move()方法在结果集中移动游标指针，使用Cursor的getString()方法获取当前记录各字段的值，并在控制台输出各条记录的结果。
...
码 8‑9 **使用 SimpleCursorAdapter 将查询结果绑定到 ListView**
...
使用ContentValues.put()分别给每个字段赋值，前一个参数是内容提供器定义的字段名称，后一个参数是给这个字段的赋值。码 8‑12 中实现了给用户字典内容提供器添加一行新记录的功能。首先创建一个新的ContentValues对象，通过put()把值分别赋给各个字段。这条数据中并没有插入\_ID字段，那是因为它会自动地被增加到数据中。内容提供器会给每一行数据赋予一个唯一的\_ID，而它往往就被看作数据库表中的主键。
...
ContentResolver.update()方法用来定义要更新的列和更新的值，如果要清除某一列的内容，使用ContentValues.putNull()方法将此值设为null。码 8‑13 的例子中实现了对用户字典内容提供器表中记录的条件更新，选出“word”字段值中以“en”开头的记录，把其“local”字段更新为null。
...
下面的代码示例聚焦删除条件的写法：目标是删除“word”列中以“in”开头的记录，而 `ContentResolver.delete()` 的返回值则表示实际删除的行数。码 8‑14 演示的正是这种“构造筛选条件并执行删除”的过程。
...
码 8‑15 中的例子，通过使用批处理模式对ContactsContract执行插入操作，说明了如何使用ContentResolver.applyBatch()方法。ContactsContract是安卓为通讯录提供的一组内容提供器契约接口。从安卓
...
表中的一行表示一个联系人，它是RawContacts表中的一行或多行数据的组合，这些RawContacts表中的行表示同一个人的不同帐户信息。Contacts中的数据由系统组合RawContacts表中的数据自动生成。码 8‑15 实现了从用户界面获取信息，然后把相应的信息插入通讯录的功能。因为插入涉及Data、RawContacts和Contacts三个表，所以使用批处理来执行。
...
Intent 可以提供对内容提供器的间接访问。即使应用没有访问权限，也可通过以下方式允许用户访问提供程序中的数据：从拥有权限的应用中返回结果
Intent，或者激活拥有权限的应用并允许用户使用该应用。即使没有适当的访问权限，也可通过以下方式访问内容提供器中的数据：将 Intent
...
前面已经介绍了内容提供器的基本概念和访问方式。接下来的问题是：什么时候真的需要自己实现一个内容提供器？答案并不是“只要用了数据库就应该建一个”。实际上，内容提供器的创建成本并不低，只有在确实需要统一对外暴露数据访问接口时，它才值得被引入。

在决定是否创建内容提供器之前，至少应先判断几件事：这些数据是否需要被其他应用访问；是否需要为搜索、桌面小部件、复制粘贴或系统框架提供标准化数据入口；是否需要把底层存储实现与外部调用方彻底解耦。如果数据只在本应用内部使用，通常直接使用 Room、文件或仓库层封装就足够了，并不一定要额外增加内容提供器。
...
码 8‑16 示例了UriMatcher的方法如何完成模式的匹配。这段代码中，针对表的资源标识符与单行的资源标识符实现了不同的处理方式，其中定义content://\<authority\>/\<path\>模式为表，定义content://\<authority\>/\<path\>/\<id\>模式为单行。UriMatcher的addURI()方法把资源名和路径映射到一个整数。match()方法返回了对应资源标识符的整数。然后通过一个switch语句根据不同整数来对应不同的模式，选择查询表或者单个记录。addURI()方法的参数说明如下：
...
码 8‑17 **在 getType() 中根据 URI 模式返回 MIME 类型**
...
onCreate()里有耗时太多的任务，会减慢内容提供器的启动，也就会减慢其对其它应用程序的响应。下面的代码示例聚焦内容提供器初始化阶段：先在 `ContentProvider.onCreate()` 中创建 `SQLiteOpenHelper`，再把数据库真正的创建延迟到首次调用 `getWritableDatabase()` 时。
...
示例代码：实现 ContentProvider.onCreate() 并延迟创建数据库

下面的代码示例对应 SQLiteOpenHelper.onCreate()：它的职责是完成底层数据表初始化，为后续 ContentProvider 真正访问数据库做好准备。
...
码 8‑18 **使用 SQLiteOpenHelper.onCreate() 创建底层数据表**
...
"image/gif"}，如果应用程序仅仅需要文件.jpg，调用ContentResolver.getStreamTypes()方法的时候使用过滤字符串\*/jpeg，则返回的结果为{"image/jpeg"}。如果内容提供器中没有支持过滤字符串的MIME类型，getStreamTypes()方法返回null。针对不同类型的存储方式，安卓系统的存储安全和有效的权限的要点有下面几个。
  ...
  - 默认情况下，保存到外部存储的数据文件是公用且可全局读取的数据文件。无法使用内容提供器来限制对外部存储内文件的访问，因为其他应用可使用其他
...
下面介绍一个内容提供器实例。为了更好了理解设计内容提供器的原则，使用一个完整的例子来讲解创建一个内容提供器的过程。这个例子的内容提供器创建，是基于上一章的SQLite学生信息数据库，这个数据库包括两张表students和departments，这两个表之间有外键约束。下面按照前一节的设计过程，逐步进行完成内容提供器的创建。
...
码 8‑19 **学生示例的内容 URI 定义**

这个定义在Contract类中实现。

（3）定义资源标识符模式

下面的代码先定义学生表与院系表相关常量，再通过 `UriMatcher` 注册 URI 模式，用于区分“整表访问”和“单行访问”（见码 8‑20）。
...
码 8‑20 **使用 UriMatcher 注册学生与院系资源模式**

（4）定义合约类

根据内容提供器的需要，定义 `Content URI`、表名、列名和 MIME 类型等常量（见码 8‑21）。
...
码 8‑21 **StudentsContract.java：定义内容 URI 与数据列常量**
...
码 8‑22 **示例内容提供器的四种 MIME 类型**

这四个 MIME 类型在 StudentsContract 类中定义为常量（见码 8‑21）。

（6）定义内容提供器

下面的 `StudentsProvider` 示例把 URI 匹配、数据库访问和 CRUD 方法集中在同一个内容提供器子类中，便于把前面分散介绍的设计点串起来理解（见码 8‑23）。
...
码 8‑23 **StudentsProvider.java：实现示例内容提供器**
...
码 8‑24 **在 Manifest 中注册 StudentsProvider**

## 8.4 数据加载

查看数据库的查询结果或通过内容提供器获取数据时，每次查询的结果并不确定，因此需要实现数据的动态加载。安卓针对这一类的数据提供了一个机制，叫数据绑定。通过数据绑定，可以把动态数据与称为AdapterView的图形控件连接起来，并自动根据数据内容进行布局调整，按照某种规则显示给用户。在数据源和AdapterView之间起连接作用的类，在安卓系统中称为适配器。当想用合适的方式显示并操作一些数据（如数组、链表、数据库结果等）时，可以使用安卓提供的适配器视图（AdapterView），这种方式叫数据绑定（见图 8‑6）。
...
首先设计用户图形界面的布局文件，把显示数据结果的 Spinner 控件作为界面中的一个组件（见码 8‑25）。
...
码 8‑25 **布局文件 c07\_spinner.xml**

2.  在活动中获得Spinner控件对象

在定义用户界面的活动子类中，下面的代码示例完成界面层绑定：在 `onCreate()` 中通过码 8‑25 所定义的 `Spinner` id 获取控件对象，为后续装配适配器做准备。

setContentView(R.layout.c07\_spinner);

Spinner s = (Spinner) findViewById(R.id.spinner);

码 8‑26 **在活动中获取 Spinner 控件对象**

（3）实现Spinner与数据源的数据绑定

获取Spinner对象后，在此活动子类的onCreate()方法内实现数据绑定。

首先为 Spinner 控件创建适配器，获得 arrays.xml 资源文件中的数组 planets；接下来将数据显示界面声明为 `android.R.layout.simple_spinner_item` 布局模式，并把适配器与控件对象绑定。这个模式是为 Spinner 类预定义好的布局模式。然后再设置下拉菜单的显示样式（见码 8‑27）。
...
在资源文件 arrays.xml 中定义数组 planets，并设置数组值（见码 8‑28）。
...
码 8‑28 **arrays.xml：定义 Spinner 的行星数据源**

运行这个例子程序，就得到图 8‑7 所示的界面。这里的数据源，也就是arrays中定义的planets数组，通过ArrayAdapter这个适配器，与Spinner这个图形显示控件联系起来，使数组数据直接按照列表的形式显示，不必再做布局的设计。这个功能就是填充布局的作用。
...
图 8‑8 ListView 和 GridView 显示效果
...
码 8‑29 **StudentsContract.java：定义学生与院系内容契约**
...
码 8‑30 **MainActivity.java：查询内容提供器并绑定到 ListView**

## 8.5 小结

本章围绕内容提供器这一基础组件展开，说明了它为什么存在、如何通过 URI 标识数据、如何借助 ContentResolver 完成查询、插入、更新和删除，以及如何通过契约类、权限控制和数据绑定把数据访问接口组织得更规范。

从工程视角看，内容提供器的核心价值在于“统一访问协议”。它把底层数据来自文件、SQLite 还是其他存储形式的差异隐藏起来，对外提供一致的 CRUD 接口和进程间访问能力。理解这一点后，读者就不容易把内容提供器误解成单纯的数据库封装，而能更准确地把它放到安卓组件协作体系中理解。
