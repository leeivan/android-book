...
本章围绕安卓应用在“后台如何工作、如何与用户保持信息同步”这两个问题展开。读者将先理解进程、线程和消息循环等多任务基础，再学习服务、绑定通信和通知管理，建立对后台执行模型的整体认识。

> 版本说明（截至 2026 年）：本章在保留 Handler、Service 等基础机制讲解的同时，补充了 Coroutine、WorkManager、通知渠道、通知权限、前台服务类型等现代实践与版本约束。

## 6.1 基本概念

当应用的第一个组件被启动时，安卓系统通常会为该应用创建一个新的 Linux 进程，并在其中建立主线程（也称界面线程）。这个线程负责实例化活动、分发输入事件、执行大部分界面回调，并驱动用户可直接感知的交互过程。只要应用的大部分组件仍运行在同一进程中，它们就默认共享这一条主线程。

这意味着，安卓应用并不是“天然多线程”的。默认情况下，新的活动、服务和广播接收器并不会自动得到独立线程；如果把网络访问、数据库查询或大文件 I/O 直接放到主线程中，就会阻塞界面更新，最终导致卡顿甚至 ANR。也正因为如此，理解进程、线程和消息循环，不只是为了会用几个 API，而是为了知道哪些工作应当留在主线程、哪些工作必须转移到后台执行。
...
码 6‑1 CustomThread.java

通过码 6‑1 的设置，工作线程CustomThread就具有了消息队列和消息循环的处理机制，可以在Handler中进行消息处理。代码中定义的Handler对象，其作用是把消息加入特定的消息队列中，并分发和处理该消息队列中的消息。
...
服务是安卓的四大组件之一，用于承载那些不直接提供界面、但又需要在组件之外继续存在一段时间的工作。它常被用于播放音乐、维持连接、执行用户可感知的长任务，或者向外部组件暴露一组可绑定调用的能力。服务本身没有用户界面，也不能自行启动，必须由其他组件通过 `startService()`、`bindService()` 或对应的现代替代机制触发。

理解服务时，最容易产生的误区是把“服务”直接等同于“后台线程”。实际上，`Service` 只是组件模型，不会自动创建工作线程；默认情况下，服务的回调仍然运行在应用主线程中。如果把耗时操作直接写进服务回调里，同样会阻塞界面。因此，服务真正解决的是“组件生命周期和能力暴露”的问题，而不是自动替你解决并发问题。

从职责上看，服务常见的用途主要有两类：一类是承载需要持续运行、且用户能够感知其存在的任务；另一类是为本应用或其他应用提供可绑定调用的接口。围绕这两类职责，服务通常表现为启动型服务和绑定型服务两种使用方式；如果涉及跨进程调用，则还会进一步延伸到远程服务与 Binder/Messenger 等通信机制。
...
虽然服务的生命周期比活动更少直接暴露在用户面前，但它反而更需要被开发者严格控制。原因很简单：服务一旦在后台持续运行，就会直接影响耗电、资源占用、任务可达性以及系统是否会出于回收资源而终止进程。学习服务生命周期时，重点应放在“服务因何而生、何时该停、被系统回收后如何恢复”这几个问题上。

服务的生命周期主要取决于其创建方式。使用 `startService()`（或现代受限场景下的前台服务启动）时，服务会在启动后独立存在，直到显式停止；使用 `bindService()` 时，服务的存活则依赖于是否仍有客户端绑定。两种方式可以单独使用，也可以组合使用，因此理解它们的差异是后续设计服务行为的前提。
...
安卓系统只有在内存不足，并且为用户提供界面响应而必须释放系统资源时，才会强制终止一个服务。如果服务是被一个正在与用户进行交互的活动绑定，那么它被杀死的可能性很小；如果这个服务被声明运行在前台，那么其也几乎不能被杀死。但是，如果这个服务被启动并且长时间运行，那么随着时间的推移系统会降低它在后台任务列表中的位置，并且这个服务将很容易被杀死；如果服务是组件通过startService()方法启动的，那么我们必须把它设计成能够在系统重启服务时妥善恢复。当然，这依赖于onStartCommand()方法的返回值。像活动一样，服务也有生命周期回调方法，可以通过实现这些回调方法来监测服务内部状态的改变，并在合适的时机执行工作。码 6‑6 示例了每个生命周期的回调方法。
...
如果任务不适合 WorkManager（例如需要与前台通知深度协作，或需要更细粒度地控制服务生命周期），可以定义 Service 的子类自行管理线程与队列。码 6‑11 示例展示了继承 Service 的方式：它对每个启动请求分发后台处理逻辑，并在处理完成后主动结束对应请求。
...
码 6‑11 HelloService.java

在码 6‑11 中，服务创建时会调用onCreate()方法。在onCreate()方法中创建Handler线程(HandlerThread)后启动此线程，然后获取当前线程的Looper对象来初始化服务的mServiceLooper，并创建mServicehandler对象。当一个组件通过调用startService()方法请求启动一个服务时，系统会调用这个服务的onStartCommand()方法。对于每一个启动服务的请求，都会产生一条带有startId和Intent参数的Message，并发送到MessageQueue中。ServiceHandler 通过继承Handler来实现服务请求的多任务处理。继承Service类实现服务比WorkManager方案需要更多手动管理工作，但也带来更高控制力。在码 6‑11 的例子中没有同时并行多个请求；如果业务需要并行处理，可以为每个请求创建新的线程并立即运行，而不必等待前一个请求完成。请注意，onStartCommand()
...
如果只在应用程序的局部使用服务，并且不需要跨进程工作，程序员可以实现自己的Binder类，用它直接为客户端提供访问服务。通常，客户端和服务端只是在同一个应用和进程中工作，不为第三方应用程序提供服务。例如，对于一个需要良好工作的播放音乐的应用程序，就需要把在后台工作的播放音乐的服务与应用自己的一个活动绑定。使用继承Binder类来定义服务的IBinder接口的步骤如下：
  ...
  - (1)在服务中创建一个Binder的实例：
      ...
      - 这个实例返回当前Service对象，其包含客户端可以调用的公开方法；

      - 这个实例返回Service类中的一个类对象，而这个类对象包含客户端可以调用的公开方法。

  - (2)在Service类的onBind()方法中返回这个Binder实例。

  - (3)在客户端的onServiceConnected()方法中获得这个Binder实例，并通过这个Binder实例调用服务端的公开方法。

服务端和客户端必须在同一个应用中，原因是客户端能够转换返回的对象，并正确调用自己的 API。服务端和客户端也必须位于同一个进程中，因为这种技术不执行任何跨进程处理。码 6‑13 是通过继承Binder绑定服务的一个简单例子。
...
码 6‑13 **LocalService.java**

在码 6‑13 中，LocalBinder对象给客户端提供了getService()方法，并在LocalService
的成员变量定义时，通过实例化
IBinder变量返回当前服务的实例，使得客户端能够借助这个方法获取LocalService服务的当前实例。这样就允许客户端调用服务中的公开方法。完成服务本身的定义后，第三步就是从客户端绑定已经定义完成的服务。所谓客户端，就是需要与服务绑定的活动之类的组件，需要在组件中完成以下几项工作：
...
码 6‑14 中，活动代码绑定了LocalService服务，并且在点击一个按钮时调用了服务公开方法 getRandomNumber()。
...
码 6‑14 **BindingActivity.java**

码 6‑14 显示了客户端怎样使用ServiceConnection接口和onServiceConnected回调方法的实现来绑定服务。

![](media/chapter06_multitasking_and_notifications/media/image7.png)

图 6‑7 绑定服务时客户端与服务端的连接过程
...
使用这种方式时，客户端并不直接调用服务端的任何方法，而是向服务端发送Message对象，由服务端在自己的处理器中接收并处理这些消息。下面的例子说明了如何使用Messenger接口定义绑定服务，见码 6‑15。
...
public class MessengerService extends Service {
    ...
    */
    private void showNotification() {
        ...
        .setContentTitle(getText(R.string.local_service_label)) // the label
        of the entry
...
码 6‑15 **MessengerService.java**

码 6‑15 中，IncomingHandler 类里定义的handleMessage()方法能够接收客户端传入的Message对象，并根据Message的“what”成员属性作出判断。客户端需要做的所有工作，就是基于服务端返回的IBinder对象创建一个Messenger对象，并使用这个Messenger对象的send()方法发送消息。下面的例子是一个简单的活动代码，它作为客户端绑定服务，并给服务发送MSG\_SAY\_HELLO消息，见码 6‑16。
...
图 6‑8 Messenger 服务的双向通信结构

## 6.4 通知管理

通知是应用与用户进行系统级沟通的重要渠道。它的作用不只是“弹出一个图标”，而是把需要用户感知或稍后处理的信息，通过系统统一的交互层交付给用户。典型场景包括消息到达、任务完成、下载进度、导航提醒、媒体播放控制以及需要用户立即关注的前台服务状态。
...
提醒式通知会在应用发出通知后立即出现，稍后便会消失，但仍照常显示在抽屉式通知栏中。例如，以下情况可能会触发提醒式通知：
  ...
  - 通知的优先级很高，且在搭载安卓 7.1（API 级别 25）及更低版本的设备上使用铃声或振动。

  - 在搭载安卓 8.0（API 级别 26）及更高版本的设备上，通知渠道的重要程度比较高。

从安卓 5.0 开始，通知可以显示在锁定屏幕上（图 6‑13）。可以采用编程方式设置应用在安全锁定屏幕上所发布通知的详情可见等级，甚至可以设置通知是否显示在锁定屏幕上。用户可以通过系统设置来选择锁定屏幕通知的详情可见等级，包括选择停用所有锁定屏幕通知。从安卓 8.0
开始，用户可以选择停用或启用各个通知渠道的锁定屏幕通知。在搭载安卓 8.0（API 级别
...
<p>图 6‑14 通知标志和长按菜单</p></td>
<td><p><img src="media/chapter06_multitasking_and_notifications/media/image15.png" style="width:2.63518in;height:0.77083in" /></p>
<p>图 6‑15 包含基本详情的通知</p></td>
</tr>
</tbody>
</table>

通知的设计由系统模板决定，应用只需要定义模板中各个部分的内容即可。通知的部分详情仅在展开后的视图中显示。图 6‑15 展示了通知最常见的部分，具体如下所示：
...
安卓建议使用系统模板，以确保设计能够很好地兼容所有设备。不过，如有必要可以创建自定义通知布局。尽管并非强制要求，但每个通知都应在被点按时打开相应的应用活动。除了这种默认的通知操作之外，还可以添加可在通知中完成与应用相关任务的操作按钮，通常不需要打开活动，如图 6‑16 所示。

<table>
<tbody>
<tr class="odd">
<td><p><img src="media/chapter06_multitasking_and_notifications/media/image16.png" style="width:2.31944in;height:0.86889in" /></p>
<p>图 6‑16 包含操作按钮的通知</p></td>
<td><p><img src="media/chapter06_multitasking_and_notifications/media/image17.png" style="width:1.84028in;height:1.84028in" /></p>
<p>图 6‑17 适合更多文本的展开式通知</p></td>
</tr>
</tbody>
</table>

从安卓 7.0（API 级别 24）开始，还可以添加直接在通知中回复消息或输入其他文字的操作。从安卓 10（API 级别
...
默认情况下，通知的文字内容会被截断以放在一行。如果需要长一些的通知，可以通过应用其他模板启用更大的展开式文本区域，如图 6‑17 所示。还可以使用图片、收件箱样式、聊天对话或媒体播放控件来创建展开式通知。

为了使用户在后续更新时不会遭遇多个通知或多余的通知轰炸，不妨考虑更新现有通知（而不是发出新通知），或者考虑使用收件箱样式的通知来显示会话更新。不过，如有必要发出多个通知，则应将这些孤立的通知分为一组（图 6‑18）（可在搭载安卓 7.0
...
<p>图 6‑18 收起和展开后的通知组</p></td>
<td><p><img src="media/chapter06_multitasking_and_notifications/media/image19.png" style="width:2.80556in;height:2.37513in" /></p>
<p>图 6‑19 时钟应用及该应用的某个渠道的通知设置</p></td>
</tr>
</tbody>
</table>

从安卓 8.0（API 级别
26）开始，必须为所有通知分配渠道，否则通知将不会显示。通过将通知归类到不同的渠道中，用户可以停用应用的特定通知渠道（而非停用所有通知），还可以控制每个渠道的视觉和听觉选项，所有这些操作都在安卓系统设置中完成（如图 6‑19 所示）。用户还可以长按通知以更改所关联渠道的行为。在搭载安卓 7.1（API 级别
...
3 种不同的级别：
  ...
  - 仅限优先事项：用户可以配置哪些系统级类别可以打扰他们（如仅限闹钟、提醒、活动、来电或短信等）。对于短信和来电，用户还可以选择根据发送者或来电者进行过滤（图 6‑21）。
...
在用户未使用应用时，通知可向他们及时提供有关应用事件的简短信息。本节介绍如何针对安卓 4.0（API 级别
14）及更高版本创建具备各种功能的通知。注意，本节的代码使用了安卓支持库中的 NotificationCompat API。这些API允许添加仅在较新版本安卓上可用的功能，同时仍向后兼容安卓 4.0（API
...
首先，需要使用 NotificationCompat.Builder 对象设置通知内容和渠道。以下示例显示了如何创建包含下列内容的通知：
  ...
  - 通知优先级，通过 setPriority() 设置。优先级确定通知在安卓 7.1 和更低版本上的干扰程度。对于安卓 8.0
    和更高版本，必须设置渠道重要性。
...
注意，NotificationCompat.Builder 构造函数要求提供渠道 ID。这是兼容安卓 8.0（API 级别
...
必须先通过向 createNotificationChannel() 传递 NotificationChannel 的实例在系统中注册应用的通知渠道，然后才能在安卓 8.0
...
由于必须先创建通知渠道，然后才能在安卓 8.0
及更高版本上发布任何通知，因此应在应用启动时立即执行这段代码。反复调用这段代码是安全的，因为创建现有通知渠道不会执行任何操作。注意，NotificationChannel 构造函数需要一个“importance”，它会使用 NotificationManager 类中的一个常量。此参数确定出现任何属于此渠道的通知时如何打断用户，但还必须使用 setPriority() 设置优先级，才能支持安卓 7.1
...
当用户触发通知操作时，系统会提示用户输入回复，如图 6‑24 所示。如需从通知回复界面接收用户输入，调用 RemoteInput.getResultsFromIntent() 并传入 BroadcastReceiver 收到的 Intent：
...
图 6‑25 操作进行中与完成后的进度通知

如果可以估算操作在任何时间点的完成进度，应通过调用 setProgress(max, progress,
false) 使用指示器的“确定性”形式（如图 6‑25 所示）。第一个参数“max”是完成值（如
...
// worker thread
// To show progress, update PROGRESS_CURRENT and update the
notification with:
// builder.setProgress(PROGRESS_MAX, PROGRESS_CURRENT, false);
// notificationManager.notify(notificationId, builder.build());
...
注意：包含全屏意图的通知有很强的干扰性，因此这类通知只能用于最紧急的时效性消息。注意：如果应用的目标平台是安卓 10（API 级别
...
在特定情况下，应用可能需要立即引起用户的注意，例如闹钟正在响铃或有来电时。以前可能已出于此目的配置了应用，也就是当应用在后台运行时启动活动。要在运行安卓 10（API
...
如需使该图片仅在通知收起时显示为缩略图（如图 6‑26 所示），调用 setLargeIcon() 并向其传递图片，同时调用
...
<p>图 6‑26 NotificationCompat.BigPictureStyle</p></td>
<td><p><img src="media/chapter06_multitasking_and_notifications/media/image17.png" style="width:1.84028in;height:1.84028in" /></p>
<p>图 6‑27 NotificationCompat.BigTextStyle</p></td>
</tr>
</tbody>
</table>

应用 NotificationCompat.BigTextStyle，以在通知的展开内容区域显示文本（图 6‑27）：
...
应用 NotificationCompat.MessagingStyle 可显示任意人数之间依序发送的消息（图 6‑28）。这是即时通讯应用的理想之选，因为它通过单独处理发送人姓名和消息文本为每条消息提供一致的布局，而且每条消息可以在多行中显示。如需添加新消息，调用
...
以添加显示在会话上方的标题。这可能是用户创建的群组名称，也可能是会话参与者的列表（如果没有具体名称）。不要为一对一聊天设置会话标题，因为如果有此字段，系统会将其当做表明会话是群组的提示。此样式仅适用于搭载安卓 7.0（API
级别 24）及更高版本的设备。如上所示，使用兼容性内容库 (NotificationCompat) 时，采用 MessagingStyle
的通知会自动回退到支持的扩展通知样式。在为聊天会话创建此类通知时，还应添加直接回复操作。

应用 NotificationCompat.MediaStyle 可显示媒体播放控件和曲目信息（图 6‑29）。最多可调用
...
从安卓 7.0（API 级别 24）开始，可以在一个通知组中显示相关通知（以前称为“捆绑式”通知）（图 6‑18）。例如，如果应用针对收到的电子邮件显示通知，应将所有通知放入同一个通知组，以便它们可以收起。为了支持较低版本，还可以添加摘要通知，该摘要通知会单独显示以总结所有单独的通知
...
从安卓 8.0（API 级别
26）开始，所有通知都必须分配到相应的渠道。对于每个渠道，可以设置应用于其中的所有通知的视觉和听觉行为。然后，用户可以更改这些设置，并确定应用中的哪些通知渠道应具有干扰性或应该可见。可以在系统设置中找到每个应用的通知渠道的用户设置（如图 6‑19 所示）。注意：界面将通知渠道称作“类别”。创建通知渠道后，便无法更改通知行为，此时用户拥有完全控制权。不过，仍然可以更改渠道的名称和说明。

应该为需要发送的每种不同类型的通知各创建一个渠道。还可以创建通知渠道来反映应用的用户做出的选择。例如，可以为用户在短信应用中创建的每个会话组设置不同的通知渠道。如果以安卓 8.0（API
级别 26）为目标平台，则必须实现一个或多个通知渠道。如果 targetSdkVersion 设置为 25
或更低级别，那么当应用在搭载安卓 8.0（API 级别
26）或更高版本的设备上运行时，其行为方式与在搭载安卓 7.1（API 级别
25）或更低版本的设备上相同。注意：如果以安卓 8.0（API 级别
26）为目标平台，并在未指定通知渠道的情况下发布了一条通知，则该通知不会显示且系统会记录错误。注意：可以在安卓 8.0（API
级别 26）中启用一个新设置，以便当以安卓 8.0（API 级别
26）为目标平台的应用尝试在无通知渠道的情况下发布通知时，在屏幕上显示一个消息框形式的警告。如需为搭载安卓 8.0（API
...
注意：应该通过对 SDK\_INT 版本设置条件来防护此代码，以使其仅在安卓 8.0（API 级别
...
的五个重要性级别之一。为渠道指定的重要性级别会应用到在其中发布的所有通知消息。如需支持搭载安卓 7.1（API
...
### 6.4.7 Android 13+ 通知权限与 Android 12+ PendingIntent 要求
...
### 6.4.8 Android 14+ 前台服务与精确闹钟注意事项
...
从 8.0（API 级别
26）开始，当关联的应用有活动通知时，启动器图标上会显示通知标志（也称为通知圆点）。用户可以长按应用图标以显示通知（以及任何应用快捷方式），如图 6‑14 所示。默认情况下，这些圆点会显示在支持圆点的启动器应用中，应用无需执行任何操作。然而，在某些情况下，可能不希望显示通知圆点，或者想要精确控制要在其中显示哪些通知。在某些情况下，通知并不适合显示标志，可以通过对
...
默认情况下，长按菜单上显示的数字会随着通知数量递增（如图 6‑14 所示），但可以针对自己的应用替换此数字。例如，如果仅使用一个通知表示多条新信息，但希望此处的计数代表新信息总数，这种情况下，替换该数字可能会很有用。如需设置自定义数字，对通知调用
setNumber()，如下所示：
...
RemoteViews(getPackageName(), R.layout.notification_large);
...
setStyle()。注意：建议不要使用未经装饰的通知，因为这会使通知与它的其余部分不匹配，进而导致通知在向通知区域应用了不同样式的不同设备上显示时，出现严重的布局兼容性问题。如需支持低于安卓 4.1（API
级别 16）的安卓版本，还应调用 setContent()，向其传递同一 RemoteViews 对象。

## 6.5 小结

本章从线程与进程模型出发，介绍了安卓如何通过消息队列、Looper 与 Handler 机制组织多任务执行，并说明了线程池、HandlerThread、协程等常见并发方案各自适用的场景。理解这些内容的核心目的，是避免把耗时工作放在主线程中，进而降低 ANR 和界面卡顿的风险。

在组件层面，本章进一步讲解了服务和通知。服务适合承载需要在界面外持续执行的任务，但新版本安卓对后台启动和前台服务使用都有更严格的限制；通知则承担把任务状态、提醒信息和用户操作入口传递给用户的职责。结合通知渠道、`POST_NOTIFICATIONS` 权限以及 `PendingIntent` 约束来理解这些机制，才能在现代 Android 平台上实现既合规又稳定的后台体验。
