...
> 版本说明（截至 2026 年 2 月）：本章以 Android 16（API 36）为基线，重点覆盖分区存储（Scoped Storage）、Photo Picker、MediaStore/SAF 与 Jetpack DataStore 等现代数据与文件存储实践。
...
private static final long NUM_BYTES_NEEDED_FOR_MY_APP = 1024 *
1024 * 10L;
...
```text
// Need the READ_EXTERNAL_STORAGE permission if accessing video files
that your
// app didn't create.
