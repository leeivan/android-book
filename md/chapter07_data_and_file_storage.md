...
存储仅供应用使用的文件，可以存储到内部存储卷中的专属目录或外部存储空间中的其他专属目录。使用内部存储空间中的目录保存其他应用不应访问的敏感信息。
  ...
  - 所需权限： 从内部存储空间访问不需要任何权限，如果应用在搭载安卓 4.4（API 级别
...
<!-- end list -->
  ...
  - 媒体
      ...
      - 所需权限：在安卓 13（API 级别 33）及更高版本中，按媒体类型申请权限：图片
        `READ_MEDIA_IMAGES`、视频 `READ_MEDIA_VIDEO`、音频 `READ_MEDIA_AUDIO`；在安卓 12（API 级别 32）及更低版本中使用
...
在新版安卓中，权限模型已从“按路径授权”转向“按数据类型和用途授权”：
  ...
  - 安卓 13（API 33）及以上：访问共享媒体应按类型申请 `READ_MEDIA_IMAGES`、`READ_MEDIA_VIDEO`、`READ_MEDIA_AUDIO`。
  - 安卓 11（API 30）及以上：`WRITE_EXTERNAL_STORAGE` 对大多数场景不再提供额外访问能力。
  - `MANAGE_EXTERNAL_STORAGE` 仅适用于文件管理器、备份还原、杀毒等核心场景，且会受到应用商店严格审核。

这种基于用途的存储模型可增强用户隐私保护，因为应用只能访问其实际需要的数据范围。

为了让用户更好地管理自己的文件并减少混乱，以安卓 10（API 级别
...
访问和存储文件。为确保应用的性能不受影响，请勿多次打开和关闭同一文件。下面的代码示例演示最基本的 File API 用法：
...
FileOutputStream。下面的代码示例展示如何将一段文本写入应用私有文件：
...
fileList() 会返回 filesDir 目录下的文件名数组。下面的代码示例只展示这一查询动作本身：
...
SD 卡后，就无法访问其中的文件。如果应用的功能取决于这些文件，应改为将文件存储在内部存储空间中。在搭载安卓 9（API 级别
...
注意：如果应用在搭载安卓 4.3（API 级别
18）或更低版本的设备上使用，则该数组仅包含一个表示主外部存储卷的元素。如需从外部存储设备访问应用专用文件，请调用
getExternalFilesDir()。为确保应用的性能不受影响，请勿多次打开和关闭同一文件。下面的代码示例展示如何定位应用专属的外部文件目录：
...
如果媒体文件只对当前应用有意义，更合适的做法是把它放在外部存储空间中的应用专属目录。下面的代码示例展示了如何定位并创建这类目录：
...
allocateBytes()。否则，应用可以请求用户从设备移除一些文件或从设备移除所有缓存文件。下面的代码示例展示如何查询设备当前可用空间：
...
如需查找满足一组特定条件（例如时长为 5 分钟或更长时间）的媒体，关键是构造类似 SQL 的选择语句。下面的代码示例展示这一筛选条件的写法：
...
loadThumbnail() 并传入目标缩略图尺寸。下面的代码示例只聚焦“按 URI 读取缩略图”这一动作：
...
如果后续处理链路更适合基于文件描述符工作，可以采用下面的打开方式：
...
如果后续处理更适合流式读取，可以改用文件流方式打开媒体。下面的代码示例展示这一分支：
...
为了帮助应用更顺畅地使用第三方媒体库，安卓 11（API 级别 30）及更高版本允许使用 MediaStore API 以外的 API
...
API 访问文件但没有必要的权限，就会发生 FileNotFoundException。如需在搭载安卓 10（API 级别
29）的设备上访问共享存储空间中的其他文件，建议在应用的清单文件中将
requestLegacyExternalStorage 设置为 true 以暂时停用分区存储。如需在安卓 10
...
用于视频位置信息。如果应用使用分区存储，系统会默认隐藏位置信息。如需访问这些信息，完成以下步骤：
  ...
  - (2)通过调用 setRequireOriginal()，从 MediaStore 对象获取照片的确切字节，并传入照片的
    URI。下面的代码示例展示了如何基于原始 URI 构造“要求返回原始字节流”的访问请求：
...
如需读取视频元数据中的位置信息，关键是先用 MediaMetadataRetriever 绑定媒体源，再读取对应字段。下面的代码示例展示了这一基本流程，且不需要额外权限。
...
如需将媒体项添加到现有集合，调用类似于以下内容的代码。此代码段可在搭载安卓 10 或更高版本的设备上访问
...
如果应用执行可能非常耗时的操作（例如写入媒体文件），那么在处理文件时对其进行独占访问非常有用。在搭载安卓 10 或更高版本的设备上，应用可以通过将
IS\_PENDING 标记的值设为 1 来获取此独占访问权限。如此一来，只有应用可以查看该文件，直到应用将 IS\_PENDING 的值改回
0。下面的代码示例基于前面的插入流程继续展开，重点说明在 MediaStore.Audio 集合中写入较长音频文件时如何使用
IS\_PENDING 标记：
...
当应用在搭载安卓 10 的设备上存储媒体时，系统默认按照类型对媒体进行整理。例如，新图片文件默认存放在
...
Java 或基于 Koltin 的代码传入原生代码。下面的代码示例展示如何把媒体对象的文件描述符继续传给原生层：
...
来征得用户同意修改文件。下面的代码示例展示了“捕获异常并转入用户授权流程”这一关键处理方式：
...
每当应用需要修改并非由其创建的媒体文件时，都完成此过程。或者，如果应用在安卓 11
或更高版本上运行，可以允许用户向应用授予对一组媒体文件的写入权限。按照如何管理媒体文件组部分中所述的方式调用
createWriteRequest()
方法。如果应用有其他用例未包含在分区存储范围内，提交功能请求并暂时停用分区存储。如需从媒体库中移除应用不再需要的某个项目，下面的代码示例展示了典型删除流程：
...
RecoverableSecurityException，如更新媒体项部分所述。如果应用在安卓 11
...
getGeneration() 的值之前，先检查媒体库是否存在更新。如果媒体库版本已更改，执行完全同步传递。在安卓 11
...
使用相同的通用模式。用户可能会信任特定的应用来执行媒体管理，如频繁地修改媒体文件。如果应用以安卓 11
或更高版本为目标平台且不是设备的默认图库应用，则每次应用尝试修改或删除文件时，都必须向用户显示一个确认对话框。如果应用以安卓 12（API
...
READ\_EXTERNAL\_STORAGE 权限才能访问其他应用的媒体文件。在搭载安卓 9（API 级别
28）或更低版本的设备上，应用必须请求
...
介绍了存储访问框架支持的访问文件和其他文档的不同用例。另外，还介绍了如何在用户选择的位置执行操作。存储访问框架支持以下访问文件和其他文档的用例。
  ...
  - 授予对目录内容的访问权限：ACTION\_OPEN\_DOCUMENT\_TREE意图操作在安卓 5.0（API 级别
...
URI。下面的代码示例展示如何创建并触发“打开 PDF 文档”意图：
...
注意：本部分介绍的意图操作在安卓 5.0（API 级别
21）及更高版本中提供。文件管理和媒体创建应用通常在目录层次结构中管理文件组。如需在应用中提供此功能，使用
ACTION\_OPEN\_DOCUMENT\_TREE意图操作，它支持用户授予应用对整个目录树的访问权限，但在安卓 11（API 级别
...
注意：如果在使用 ACTION\_OPEN\_DOCUMENT\_TREE 访问的目录中遍历大量文件，应用的性能可能会降低。在安卓 11（API
...
此外，在安卓 11（API 级别 30）及更高版本中，不能使用
...
5
张图片，那么在用户重启设备后，就必须让用户返回到系统选择器来查找这些文件。如需在设备重启后继续保留访问权，关键是显式获取可持久化 URI 权限。下面的代码示例展示了这一动作：
...
takePersistableUriPermission()，应用也无法保留对该URI的访问权限。在此类情况下，需要再次请求权限才能重新获得对该URI的访问权限。获得文档 URI 后，下一步通常是读取元数据。下面的代码示例展示如何提取并记录这些信息：
...
获取对文档URI的引用后，可以打开文档进行进一步处理。下面的代码示例展示了在已获得 Bitmap 文件 URI 的前提下如何真正打开该文件：
...
注意：应在后台线程而非界面线程上完成此操作。打开位图后，可以在 ImageView 中显示该位图。下面的代码示例展示了如何在已获得
InputStream 对象 URI 的前提下读取文本内容：
...
FLAG\_SUPPORTS\_WRITE 的值。下面的代码示例展示了如何覆盖给定 URI 所代表文档的内容：
...
URI，例如打开文档。getMediaUri() 方法支持 ExternalStorageProvider URI。在安卓 12（API 级别
31）及更高版本中，此方法还支持 MediaDocumentsProvider URI。在安卓 7.0（API 级别
...
CATEGORY\_OPENABLE 类别。在用户做出选择后，第一步通常不是直接读取内容，而是先判断该 URI 是否对应虚拟文件。下面的代码示例展示了这个判定逻辑：
...
"image/png"。下面的代码示例展示如何判断某个虚拟文件能否表示为图片，并在可行时取得输入流：
...
应用程序通常包括允许用户修改应用程序的特性和行为的设置功能，例如一些应用程序允许用户指定通知是否启用或指定多久使用云同步数据。如果要为应用程序提供设置共享偏好的功能（见图 7‑1），需要使用安卓的偏好API来构建统一的接口。
...
从安卓 7.0（API 级别 24）开始，如果使用这些模式，安卓会抛出
...
PreferenceFragmentCompat，拥有自己单独的层次结构。然后，初始屏幕上的偏好可关联至含有相关偏好的子屏幕。图 7‑3 显示了一个简单的层次结构，它包含两个类别：“Messages”和“Sync”。图 7‑4 显示了拆分为多个屏幕的同一组偏好。如需关联含有**偏好**的屏幕，可以在
...
图 7‑5 EditTextPreference 使用 SummaryProvider 动态显示摘要
...
表 7‑1 OnPreferenceChangeListener 与 OnPreferenceClickListener 的区别
...
注释添加到类中。将注释的值设为类应该表示的查询。下面的代码示例展示了 Room 视图实体的基本定义方式：
...
默认情况下，Room 会为实体中定义的每个字段创建一个列。如果某个实体中有不想持久化的字段，可以通过 @Ignore 明确排除。下面的代码示例展示了这种声明方式：
...
扩展模块）为实体提供支持。如需使用 Room 2.1.0 及更高版本提供的这一能力，关键是把实体声明为 FTS 表。下面的代码示例展示了 @Fts3 / @Fts4 的基本用法：
...
注解中添加 indices 属性，列出要在索引或复合索引中包含的列的名称。下面的代码示例展示了这一声明过程：
...
true，强制实施此唯一性属性。下面的代码示例展示了如何禁止表中出现 firstName 和 lastName 组合值完全相同的两行记录。
...
可以正确解释这些方法的自动生成实现。下面的代码示例展示了一个使用 @AutoValue
...
User 类（带有 @Embedded 注释）中添加 Address 字段时，核心思路是把地址对象拆分成可映射到表字段的嵌套属性。下面的代码示例展示了这种定义方式：
...
有时，可能需要查询包含三个或更多表格的集合，这些表格之间互相关联。在这种情况下，需要定义各个表之间的嵌套关系。在音乐在线播放应用示例中，假设想要查询所有用户、每个用户的所有播放列表以及每个用户的各个播放列表中包含的所有歌曲。用户与播放列表之间存在一对多关系，而播放列表与歌曲之间存在多对多关系。下面的代码示例展示了这些实体以及播放列表与歌曲之间多对多关系的交叉引用表定义：
