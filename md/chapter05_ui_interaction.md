...
> 版本说明（截至 2026 年 2 月）：本章示例已按 Android 最新平台实践更新（以 Android 16 / API 36 为基线，并兼容 Android 14/15 的主流行为约束）。在新项目中，建议统一使用 `registerForActivityResult()`、系统照片选择器（Photo Picker）与受限广播注册策略（`RECEIVER_EXPORTED` / `RECEIVER_NOT_EXPORTED`）。
