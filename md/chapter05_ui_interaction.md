...
本章从“页面之间如何通信”和“应用如何响应外部消息”两条主线展开，重点介绍意图、数据传递和广播接收器。前者解决组件之间如何发起跳转、调用系统能力和携带参数，后者帮助应用在系统或其他应用发送消息时作出响应，这两部分共同构成安卓界面交互的重要基础。

> 版本说明（本章修订版）：示例已按近年 Android 平台实践更新。新项目建议优先使用 `registerForActivityResult()`、系统照片选择器（Photo Picker）与显式广播导出策略（`RECEIVER_EXPORTED` / `RECEIVER_NOT_EXPORTED`）。

## 5.1 理解意图

意图（Intent）是安卓组件之间最核心的消息描述对象。可以把它理解为“对一次操作请求的封装”：它既描述了要做什么事，也可以携带做这件事所需的数据与附加条件。当某个组件创建并发送一个意图后，系统会根据意图中携带的信息去匹配合适的目标组件，并触发对应的执行流程。

### 5.1.1 意图的概念

安卓强调组件之间的低耦合，因此系统并不鼓励页面、服务或接收器之间直接硬编码彼此的调用方式，而是通过意图这种统一的消息机制来完成协作。借助意图，应用既可以在自身内部完成页面跳转和数据传递，也可以调用其他应用已经公开的能力，例如拍照、查看地图、发送邮件或选择联系人。

从教学角度看，理解意图的关键并不只是记住几个 API 名称，而是理解它在系统中的角色：它既是一份“操作说明”，也是组件之间的一份“契约”。例如，在一个联系人列表中点击某一项时，当前页面并不需要知道目标页面是如何绘制的，只需要通过意图说明“我要查看这个联系人，并把联系人标识传过去”，系统和目标组件就会接手后续流程。下面再具体分析一个意图对象通常由哪些部分组成。
...
码 5‑1 **在 AndroidManifest.xml 中声明可接收隐式意图的活动**
...
在码 5‑5 中，可知此应用程序定义了三个活动，每一个活动都定义了多个意图模板。其中命名为com.android.notepad.NotesList的第一个活动行为进入应用程序的主入口，通过定义三个意图过滤器可以做三件事情。第一个意图过滤模板代码如下：
...
码 5‑10 **explicit\_intent\_main\_layout.xml：显式意图示例的主界面布局**
...
码 5‑11 **explicit\_intent\_second\_layout.xml：显式意图示例的第二界面布局**
...
码 5‑12 **ExplicitMainActivity.java：通过显式 Intent 跳转到第二个活动**
...
码 5‑13 **ExplicitSecondActivity.java：点击按钮返回主界面**
...
码 5‑14 **创建带自定义 Action 的隐式 Intent**
...
码 5‑15 **在 Manifest 中为自定义 Action 声明过滤条件**
...
码 5‑16 **ImplicitMainActivity.java：发送带自定义 Action 的隐式 Intent**
...
码 5‑17 **ImplicitSecondActivity.java：接收隐式 Intent 并加载界面**
...
ACTION\_SEND 操作执行“共享”时，用户根据目前的状况可能需要使用另一不同的应用，因此应当始终使用选择器对话框，如图 5‑1 所示。

![](media/chapter05_ui_interaction/media/image1.png)
...
码 5‑18 **使用 Intent.createChooser() 显示应用选择器**
...
码 5‑19 **注册 ActivityResultLauncher 并在点击时启动内容选择**
...
会注册回调，但它不会启动另一个活动并发出结果请求。这些操作由返回的
...
码 5‑20 **在 LifecycleObserver 中使用 ActivityResultRegistry**
...
码 5‑21 **使用 ActivityResultRegistry 注册结果启动器**
...
Uri> {
    ...
    public Uri parseResult(int resultCode, @Nullable Intent result) {
        ...
        return
        result.getParcelableExtra(RingtoneManager.EXTRA_RINGTONE_PICKED_URI);
    }
}
```

码 5‑22 **自定义 ActivityResultContract**
...
码 5‑23 **DataTransferFirstActivity：启动第二个活动并接收返回结果**
...
码 5‑24 **DataTransferSecondActivity：读取传入数据并回传结果**

<table>
<tbody>
<tr class="odd">
<td><p><img src="media/chapter05_ui_interaction/media/image2.png" style="width:2.625in;height:0.93056in" /></p>
<p>图 5‑2 数据传递示例中的第一个活动界面</p></td>
<td><p><img src="media/chapter05_ui_interaction/media/image3.png" style="width:2.53472in;height:0.94538in" /></p>
<p>图 5‑3 数据传递示例中的第二个活动界面</p></td>
</tr>
</tbody>
</table>
...
码 5‑25 **使用 ACTION_SET_ALARM 创建闹钟**
...
码 5‑26 **声明 ACTION_SET_ALARM 所需权限**
...
码 5‑27 **使用 ACTION_SET_TIMER 创建倒计时器**
...
码 5‑28 **为 ACTION_SET_TIMER 声明意图过滤器**
...
码 5‑29 **为 ACTION_SHOW_ALARMS 声明意图过滤器**
...
码 5‑30 **使用 ACTION_INSERT 向日历添加事件**
...
码 5‑32 **使用 ACTION_IMAGE_CAPTURE 拍照并保存到指定 URI**
...
码 5‑33 **为 ACTION_IMAGE_CAPTURE 声明意图过滤器**
...
码 5‑34 **使用 INTENT_ACTION_STILL_IMAGE_CAMERA 打开拍照界面**
...
码 5‑35 **为 INTENT_ACTION_STILL_IMAGE_CAMERA 声明意图过滤器**
...
码 5‑36 **使用 INTENT_ACTION_VIDEO_CAMERA 打开摄像界面**
...
码 5‑37 **为 INTENT_ACTION_VIDEO_CAMERA 声明意图过滤器**
...
传送至应用的 Activity Result 回调的结果意图包含指向所选联系人数据的 content:
URI。响应会为应用授予该联系人数据的临时读取权限，即使应用不具备
...
码 5‑41 **使用 ACTION_EDIT 编辑联系人并预填字段**
...
码 5‑43 **使用 ACTION_SEND 撰写带附件的电子邮件**
...
码 5‑45 **为电子邮件发送与 mailto: 声明意图过滤器**

#### 5.1.5.6 文件存储

如需让用户选择文档或照片等文件并向应用返回文件引用，使用 ACTION\_GET\_CONTENT 行为并指定所需 MIME
类型。返回给应用的文件引用对活动的当前生命周期而言是瞬态引用，因此如果想稍后继续访问，就必须导入可在之后读取的副本。用户还可利用此意图在流程中创建新文件（例如，用户可以不选择现有照片，而是用相机拍摄新照片）。传送至应用的
Activity Result 回调方法的结果意图会包含指向该文件的 URI。该 URI 可以是任何类型，如 http:
URI、file: URI 或 content: URI。不过，如果想将可选择的文件限定为可从内容提供器 (content: URI)
...
码 5‑47 **为 ACTION_GET_CONTENT 声明图片选择过滤器**

在 Android 4.4 或更高版本上运行时，可以不必检索必须导入应用的文件副本（使用 ACTION\_GET\_CONTENT
行为），而是使用 ACTION\_OPEN\_DOCUMENT 行为并指定 MIME
类型，请求打开由另一个应用管理的文件。如果还需要允许用户创建应用可写入的新文档，可改用
ACTION\_CREATE\_DOCUMENT 行为。例如，ACTION\_CREATE\_DOCUMENT 意图允许用户选择他们想在哪里创建新
PDF 文档（在另一个管理文档存储的应用内），而不是从现有文档中进行选择。应用随后会收到其可以写入新文档的 URI 位置。尽管从
ACTION\_GET\_CONTENT 行为传递至应用的 Activity Result 回调方法的意图可能返回任何类型的 URI，来自
...
会为应用授予对文件的长期读取权限（还可能会授予写入权限）。因此，如果想读取现有文件而不将其副本导入应用，或者想就地打开和编辑文件，特别适合使用
...
ClipData 对象检索每个项目。注意：这些意图必须指定 MIME 类型，并且必须声明 CATEGORY\_OPENABLE
...
类型数组来指定多个 MIME类型。如果这样做，必须将 setType() 中的主 MIME 类型设置为 "\*/\*"。
  ...
  - 附加消息：
      ...
      - EXTRA\_MIME\_TYPES：与应用请求的文件类型对应的 MIME 类型数组。当使用此附加消息时，必须在
...
码 5‑48 **使用 OpenDocument 获取可长期访问的图片 URI**
...
码 5‑49 **通过 DocumentsProvider 向系统文档界面公开文件**
...
码 5‑50 **使用 ACTION_RESERVE_TAXI_RESERVATION 发起叫车请求**
...
码 5‑56 **使用 INTENT_ACTION_MEDIA_PLAY_FROM_SEARCH 按艺术家播放音乐**
...
码 5‑57 **为 INTENT_ACTION_MEDIA_PLAY_FROM_SEARCH 声明意图过滤器**
...
protected void onCreate(Bundle savedInstanceState) {
    ...
    if
    (intent.getAction().compareTo(MediaStore.INTENT_ACTION_MEDIA_PLAY_FROM_SEARCH)
    == 0) {
        ...
        (mediaFocus.compareTo(MediaStore.Audio.Genres.ENTRY_CONTENT_TYPE) ==
        0) {
        ...
        (mediaFocus.compareTo(MediaStore.Audio.Artists.ENTRY_CONTENT_TYPE)
        == 0) {
        ...
        (mediaFocus.compareTo(MediaStore.Audio.Albums.ENTRY_CONTENT_TYPE) ==
        0) {
        ...
        (mediaFocus.compareTo(MediaStore.Audio.Playlists.ENTRY_CONTENT_TYPE)
        == 0) {
...
码 5‑58 **在 onCreate() 中解析媒体搜索意图并分发播放请求**
...
码 5‑59 **使用 ACTION_CREATE_NOTE 创建笔记**
...
码 5‑60 **为 ACTION_CREATE_NOTE 声明意图过滤器**
...
码 5‑62 **使用 ACTION_DIAL 打开拨号器**

#### 5.1.5.12 搜索

如需支持在应用内进行搜索，使用 SEARCH\_ACTION 行为并在应用中声明一个意图过滤器，如下文示例意图过滤器中所示。
...
码 5‑63 **为应用内搜索声明 SEARCH_ACTION 过滤器**
...
码 5‑64 **使用 ACTION_WEB_SEARCH 发起网页搜索**

#### 5.1.5.13 设置

如需在应用要求用户更改系统配置时打开某个设置界面，可以使用下列意图操作打开与操作名称对应的系统设置屏幕。
...
码 5‑66 **使用 ACTION_SENDTO 撰写短信或彩信**
...
码 5‑70 **为网页链接处理声明意图过滤器**

提示：如果你的安卓应用提供与网站相似的功能，可以为指向该网站的 URL
加入一个意图过滤器。如果用户安装了应用，点击电子邮件或其他网页中指向该网站的链接时，将会打开安卓应用而不是网页。
...
码 5‑71 **通过 ACTION_VIEW 与 URI 启动系统浏览器**

在下面的例子中定义了一个单选列表界面，简单调用浏览器、电话拨号、日历等系统应用程序。通过这个例子，可以了解如何利用意图启动和激活常用的系统应用程序。首先定义应用程序的界面布局，然后创建用户界面活动（见码 5‑72）。
...
码 5‑72 **ActionDataActivity.java：按 Action/Data 组合发起系统意图**

![](media/chapter05_ui_interaction/media/image4.png)

图 5‑4 ActionDataActivity 示例界面
...
广播机制是一种在系统与应用、应用与应用之间分发事件消息的方式。广播接收器（BroadcastReceiver）负责监听这类消息，并在收到匹配广播后执行相应处理。它更像一个“事件入口”而不是一个完整业务容器：广播告诉应用“发生了什么”，而接收器则决定是否以及如何继续响应。

### 5.2.1 基本概念

多数广播由系统发起，例如电量变化、时区变化、开机完成或网络状态变化；应用本身也可以发送自定义广播。需要特别注意的是，广播接收器没有用户界面，也不适合承载耗时逻辑。更合理的做法通常是：接收器先快速判断事件类型，再把真正的耗时工作交给通知、服务、WorkManager 或应用内其他机制处理。

如果应用需要发送广播，常见方式包括 `sendBroadcast()` 和 `sendOrderedBroadcast()`。前者面向普通广播，适合“一次通知多个接收者”；后者面向有序广播，适合需要按优先级依次传递并允许前序接收者影响后续结果的场景。不过在现代项目里，有序广播的使用场景已经明显减少，很多应用内事件更适合交给 `Flow/SharedFlow`、`LiveData` 或其他可感知生命周期的机制处理。应用接收广播通常有两种方式：清单声明的接收器，以及运行时通过上下文注册的接收器。
...
码 5‑73 **在 Manifest 中声明广播接收器与订阅动作**
...
码 5‑74 **MyBroadcastReceiver：记录并显示接收到的广播**
...
码 5‑75 **创建 BroadcastReceiver 实例**
...
码 5‑76 **构造 IntentFilter 并动态注册接收器**
...
广播接收器的动态注册方式采用在Java程序代码中调用活动的方法来注册，因为是在程序运行过程中才注册，所以称为动态注册。注册时所需要信息同样包括广播接收器和意图过滤器，以及行为条件。动态注册方式与静态注册方式不同，因为在程序运行中注册，所以当应用程序关闭后，广播接收器就不再进行监听。下面这个例子通过按钮事件，在Java程序中动态实现广播接收器的注册和注销，见图 5‑7。

![](media/chapter05_ui_interaction/media/image5.png)
...
②定义用户界面的活动，导入布局文件，并根据按钮不同的功能，在按钮点击事件处理代码中实现不同的功能，见码 5‑87。
...
public class RegisterBroadcastActivity extends AppCompatActivity {
    ...
    "com.example.ch05.REGISTER_ACTION";
...
码 5‑87 **RegisterBroadcastActivity.java：动态注册、注销并发送广播**

③定义广播接收器，处理广播消息，见码 5‑88。
...
码 5‑88 **TestReceiver.java：接收并输出广播内容**
...
本章主要介绍了意图和广播接收器这两类与界面交互密切相关的机制。意图用于描述要执行的动作、要访问的数据以及目标组件的信息，因此既可以用于应用内页面跳转，也可以用于跨应用调用系统能力；广播接收器则负责接收系统或应用发送的广播消息，并在满足过滤条件时执行响应逻辑。

从实践角度看，本章的关键不只是区分显式意图、隐式意图和不同广播类型，更重要的是理解“组件解耦”这一设计思想：界面之间不直接硬编码依赖，而是通过标准化消息完成协作。掌握这一点后，读者在设计页面流程、数据传递和系统集成时会更有条理。
